...
public:
    ...
    public:
        customer *curr; // a changeable head for updating X position
        int count;
        CDLL(): curr(nullptr), count(0) {}
        ~CDLL() {
            while (curr) remove(curr);
            delete curr;
        }
        void add(customer *c) { // directly add c to the updated X position, change head to that position
            if (count >= MAXSIZE / 2) {
                ...
                for (int i = 0; i < count; i++, temp = temp->next) {
                    if (abs(temp->energy - c->energy) > maxDiff) {
                        maxDiff = abs(temp->energy - c->energy);
                        curr = temp;
                    }
                }
            }
            if (!curr) {
                curr = c;
                curr->next = curr->prev = c;
            }
            else {
                if (c->energy < curr->energy) {
                    c->prev = curr->prev;
                    c->next = curr;
                } else {
                    c->prev = curr;
                ...
                curr = c->next->prev = c->prev->next = c;
            }
            count++;
        }
        void remove(customer* pos) { // delete chosen pos, update head to either side
            pos->next->prev = pos->prev;
            pos->prev->next = pos->next;
            curr = pos->energy > 0 ? pos->next : pos->prev;
            delete pos;
            if (--count == 0) curr = nullptr;
        }
    };
    class Q{
    public:
        ...
        customer* dequeue() { // take the head of the queue, can be used to put in table
            ...
            count--;
            return pos; // pos->next will be pointed to someone on the table, don't worry
        }
        void remove(customer *pos) { // delete at required position, this is the generalized version
            if (pos == head) head = head->next;
            else pos->prev->next = pos->next;
            if (pos == tail) tail = tail->prev;
    ...
    int absInsSort(customer *currHead, int size, int gap) const {
        ...
        for (int i = gap; i < size; i += gap) {
            for (int j = i; j >= gap; j -= gap) {
                ...
                if (abs(back->energy) > abs(front->energy) || (abs(back->energy) == abs(front->energy) &&
                found(front->name,found(back->name, tList->head, tList->count),tList->count))) {
                    swap(front, back);
                    result++;
                }
            }
    ...
    int absShellSort(customer* currHead, int size) const {
        ...
        for (int i = size / 2; i > 0; i /= 2) {
            if (i == 2) continue;
            for (int j = 0; j < i; j++) result += absInsSort(getNext(currHead, j), size - j, i);
        }
        return result;
    }
    ...
    void PURPLE() override {
        int maxIdx = 0, maxAbsEnergy = 0;
        customer *temp = rQueue->head;
        for (int i = 0; i < rQueue->count; i++, temp = temp->next) {
            if (abs(temp->energy) >= maxAbsEnergy) {
                maxIdx = i;
                maxAbsEnergy = abs(temp->energy);
            }
        }
        return BLUE(absShellSort(rQueue->head, maxIdx + 1) % MAXSIZE);
    }
    void REVERSAL() override {
        if (!(rTable->curr)) return;
        string tempName = rTable->curr->name;
        customer *first = rTable->curr, *last = first->next; // 2 pointers like Mr Phung's quick sort
        while (true) {
            while (first->energy < 0 && first != last) first = first->prev;
            while (last->energy < 0 && first != last) last = last->next;
            swap(first, last);
            if (first == last || first->prev == last) break;
            first = first->prev;
            last = last->next;
        }
        first = rTable->curr, last = first->next;
        while (true) {
            while (first->energy > 0 && first != last) first = first->prev;
            while (last->energy > 0 && first != last) last = last->next;
            swap(first, last);
        ...
        rTable->curr = found(tempName, rTable->curr, rTable->count);
    }
    void UNLIMITED_VOID() override {
        if (rTable->count < 4) return;
        customer *subHead = rTable->curr, *subTail = getNext(subHead,2), *minHead = subHead; // setup for finding the max-sum-sublist
        ...
        for (int currLength = 4; currLength <= rTable->count; currLength++) {
            ...
            currSum += subTail->energy; // increase length, now the currSum is the sum of at least 4 elements
        ...
        customer *temp = minHead;
        int minEnergy = maxInt, minRelativeIdx = 0;
        ...
        for (int i = 0; i < maxLength; i++)
            getNext(minHead, (minRelativeIdx + i) % maxLength)->print(); // from minIdx, print clockwise
    }
    void DOMAIN_EXPANSION() override {
        customer *temp = tList->head;
        int sum = 0;
        while (temp) {
            sum += temp->energy;
            temp = temp->next;
        ...
        while (temp) {
            if ((sum >= 0 && temp->energy < 0) || (sum < 0 && temp->energy > 0)) { // kicking phase, first to last
                customer *newTemp = temp->next;
                customer *pos; // hold the next position for temp to go if temp->name can be found elsewhere
                pos = found(temp->name, rTable->curr, rTable->count);
                if (pos) rTable->remove(pos); // must be in either queue or table
                pos = found(temp->name, rQueue->head, rQueue->count);
                if (pos) rQueue->remove(pos); // so just find and delete it
                tList->remove(temp);
                temp = newTemp;
            } else temp = temp->next;
        }
    ...
    void LIGHT(int num) override{
        cout << "light " << num << endl;
        customer *temp = (num == 0 ? rQueue->head : rTable->curr);
